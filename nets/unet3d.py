...
class UNet(nn.Module):
    ...
    def forward(self, x):
        # Down sampling
        down_1 = self.down_1(x)  # -> [1, 4, 16, 128, 128]
        pool_1 = self.pool_1(down_1)  # -> [1, 4, 8, 64, 64]

        down_2 = self.down_2(pool_1)  # -> [1, 8, 8, 64, 64]
        pool_2 = self.pool_2(down_2)  # -> [1, 8, 4, 32, 32]

        down_3 = self.down_3(pool_2)  # -> [1, 16, 4, 32, 32]
        pool_3 = self.pool_3(down_3)  # -> [1, 16, 2, 16, 16]

        down_4 = self.down_4(pool_3)  # -> [1, 32, 2, 16, 16]
        pool_4 = self.pool_4(down_4)  # -> [1, 32, 1, 8, 8]

        # down_5 = self.down_5(pool_4)  # -> [1, 64, 8, 8, 8]
        # pool_5 = self.pool_5(down_5)  # -> [1, 64, 4, 4, 4]

        # Bridge
        bridge = self.bridge(pool_4)  # -> [1, 128, 1, 8, 8]
        ...
        trans_2 = self.trans_2(bridge)  # -> [1, 64, 2, 16, 16]
        concat_2 = torch.cat([trans_2, down_4], dim=1)  # -> [1, 96, 2, 16, 16]
        up_2 = self.up_2(concat_2)  # -> [1, 32, 2, 16, 16]

        trans_3 = self.trans_3(up_2)  # -> [1, 32, 4, 32, 32]
        concat_3 = torch.cat([trans_3, down_3], dim=1)  # -> [1, 48, 4, 32, 32]
        up_3 = self.up_3(concat_3)  # -> [1, 16, 4, 32, 32]

        trans_4 = self.trans_4(up_3)  # -> [1, 16, 8, 64, 64]
        concat_4 = torch.cat([trans_4, down_2], dim=1)  # -> [1, 24, 8, 64, 64]
        up_4 = self.up_4(concat_4)  # -> [1, 8, 8, 64, 64]

        trans_5 = self.trans_5(up_4)  # -> [1, 8, 16, 128, 128]
        concat_5 = torch.cat([trans_5, down_1], dim=1)  # -> [1, 12, 16, 128, 128]
        up_5 = self.up_5(concat_5)  # -> [1, 4, 16, 128, 128]

        # Output
        out = self.out(up_5)  # -> [1, 2, 16, 128, 128]
        return out